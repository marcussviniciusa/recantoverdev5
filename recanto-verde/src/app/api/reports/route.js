...
export async function GET(request) {
  try {
    ...
    // Simular dados de ocupação (você pode calcular baseado nos logs reais)
    const averageOccupancy = Math.random() * 30 + 60; // 60-90%
    ...
    const tableUtilization = tables.map(table => ({
      ...
      utilizationRate: Math.random() * 40 + 50 // 50-90%
    }));
